...
![Chat mode selector dropdown in the prompt area](../images/core-chat-modes-selector.png)

## Available Modes

### Code
- **Command**: `/code`
- **Purpose**: Direct interaction with Aider to request code changes. It's best for specific, well-defined tasks like "add a new function" or "fix this bug." Tasks start in Agent Mode by default; switch to Code when you want aider-style editing instead of the autonomous agent.

### Agent
- **Command**: `/agent`
- **Purpose**: The default mode. Activates the autonomous agent. Use this mode for high-level, complex tasks that may require multiple steps, reasoning, and the use of various tools (e.g., "implement a new API endpoint" or "refactor the user authentication flow").
