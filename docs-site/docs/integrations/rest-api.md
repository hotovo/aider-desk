...
- **SSE Response**: `200 OK` with `Content-Type: text/event-stream`
  ...
  SSE event types: `response-chunk`, `response-completed`, `tool`, `tool-input-chunk`, `log`, `ask-question`, `user-message`, `task-created`, `task-updated`. The stream ends with a `stream-end` event after `response-completed`.
...
- **Request Body**:
  ...
  | `activate` | boolean | No | Whether to activate the task after creation |
...
- `POST /api/project/worktree/add-file-to-git` — Track an untracked (new) file in the worktree with Git
- `POST /api/project/worktree/cancel-commit-changes` — Cancel an in-progress commit operation
...
curl -X POST http://localhost:24337/api/project/tasks/delete \
  -H "Content-Type: application/json" \
  -d '{
    "projectDir": "/path/to/project",
    "id": "a1b2c3d4"
