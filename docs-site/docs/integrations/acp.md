...
Run AiderDesk in headless mode via Docker. See the [Docker guide](../advanced/docker.md) for full setup instructions.
...
Install and run AiderDesk as a global npm package. See the [npm CLI guide](../advanced/npm-cli.md) for details.
...
Selects the [chat mode](../core/chat-modes.md) for the session:
...
- **Docker**: `docker start aiderdesk` (or `docker run ...` — see [Docker guide](../advanced/docker.md))
- **npm CLI**: Run `aiderdesk start` (see [npm CLI guide](../advanced/npm-cli.md))
...
- [Chat Modes](../core/chat-modes.md) — Learn about AiderDesk's chat modes
- [Agent Profiles](../agent-mode/agent-profiles.md) — Configure agent behavior and tool access
- [REST API](./rest-api.md) — The underlying API that the ACP bridge uses
- [npm CLI](../advanced/npm-cli.md) — Running AiderDesk via npm
- [Docker](../advanced/docker.md) — Running AiderDesk in Docker
