...
export const TASKS_TOOL_DESCRIPTIONS = {
  ...
  [TASKS_TOOL_CREATE_TASK]:
    "Create a new task in the current project with an initial prompt. Optionally use user-provided agentProfileId to use a different agent, a modelId to override the model, or a parentTaskId (if available) to create a subtask of another task. The new task will start with the provided prompt as its first user message. Use this to begin new work streams, separate different aspects of a project, or break down complex tasks into manageable subtasks. For worktree behavior, omit worktree to inherit the parent's working mode and worktree for subtasks or use the project default for top-level tasks. Set worktree to true to reuse the parent's worktree when available, otherwise create a new one; set it to false only to force local mode. Use executionMode to control how the task runs: create_only (default) only creates the task and saves the initial prompt without executing it; wait_for_finish starts the task immediately and waits for its prompt to complete before returning; run_in_background starts the task immediately and returns right away.",
