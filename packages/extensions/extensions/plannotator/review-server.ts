...
export const getGitContext = (): GitContext => {
...
export const runGitDiff = (diffType: DiffType, defaultBranch = 'main'): { patch: string; label: string } => {
  switch (diffType) {
    case 'uncommitted':
    ...
    case 'branch':
      return {
        patch: git(`diff ${defaultBranch}..HEAD --src-prefix=a/ --dst-prefix=b/`),
