...
export class TerminalApi extends BaseApi {
  ...
  registerRoutes(router: Router): void {
    ...
    router.get(
      '/terminal/:taskId',
      this.handleRequest(async (req, res) => {
        const terminalId = this.eventsHandler.getTerminalForTask(String(req.params.taskId));
        res.status(200).json({ terminalId });
      }),
    );
    ...
    router.get(
      '/terminal/:taskId/all',
      this.handleRequest(async (req, res) => {
        const terminals = this.eventsHandler.getTerminalsForTask(String(req.params.taskId));
