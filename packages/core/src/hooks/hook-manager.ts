...
import { Mode, ResponseCompletedData, ContextFile, TaskData, QuestionData } from '@aider-desk/common/types';
      ...
      for (const file of files) {
        if (file.endsWith('.js')) {
          ...
          try {
            ...
            const resolvedPath = _require.resolve(filePath);
            delete _require.cache[resolvedPath];
            const hookFile = _require(filePath);
            hooks.push(hookFile);
