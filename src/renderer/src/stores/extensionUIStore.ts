...
interface ExtensionUIActions {
  ...
  cleanupTaskCache: (projectDir: string, taskId: string) => void;
  cleanupProjectCache: (projectDir: string) => void;
}

type ExtensionUIStore = ExtensionUIState & ExtensionUIActions;

const DEVTOOLS_OPTIONS = {
  name: 'ExtensionUIStore',
    ...
    (set, get) => ({
      ...
      cleanupTaskCache: (projectDir, taskId) => {
        set((state) => {
          ...
          for (const key of newComponentsMap.keys()) {
            if (key.endsWith(taskSuffix) && key.includes(projectPattern)) {
              newComponentsMap.delete(key);
            }
          }
          for (const key of newDataMap.keys()) {
            if (key.endsWith(taskSuffix) && key.includes(projectPattern)) {
              newDataMap.delete(key);
              newDataLoadedMap.delete(key);
            }
          ...
          return { componentsMap: newComponentsMap, dataMap: newDataMap, dataLoadedMap: newDataLoadedMap };
        });
      },
...
export const cleanupTaskCache = (projectDir: string, taskId: string) => useExtensionUIStore.getState().cleanupTaskCache(projectDir, taskId);

export const cleanupProjectCache = (projectDir: string) => useExtensionUIStore.getState().cleanupProjectCache(projectDir);
