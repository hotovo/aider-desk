...
describe('DiffLineCommentPanel', () => {
  const mockPosition = { top: 100, left: 50 }; // viewport coordinates

  const defaultProps = {
    onSubmit: vi.fn(),
    onCancel: vi.fn(),
    anchorRect: mockPosition,
  };
  ...
  it('does not render create new task checkbox', () => {
    render(<DiffLineCommentPanel {...defaultProps} />);

    expect(screen.queryByRole('checkbox')).not.toBeInTheDocument();
  });

  it('pre-populates textarea with initialText', () => {
    render(<DiffLineCommentPanel {...defaultProps} initialText="Existing comment" />);

    const textarea = screen.getByPlaceholderText('Describe the changes you want to make to this code...');
    expect(textarea).toHaveValue('Existing comment');
  });

  it('submits edited text from initialText', async () => {
    render(<DiffLineCommentPanel {...defaultProps} initialText="Original text" />);
