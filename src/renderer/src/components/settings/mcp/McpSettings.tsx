...
export const McpSettings = ({ mcpServers, setMcpServers, openProjects = [], selectedMcpContext }: Props) => {
    ...
    void Promise.all(
      Object.entries(servers).map(async ([name, config]) => {
        try {
          ...
          if (!cancelled) {
            setServersTools((prev) => ({
              ...prev,
              [name]: { tools: loadedTools, loading: false, error: null },
            }));
          }
        } catch (loadError) {
          if (!cancelled) {
            ...
            setServersTools((prev) => ({
              ...prev,
              [name]: { tools: null, loading: false, error: errorMessage },
            }));
          }
        }
      }),
          ...
          projectDir
            ? {
                ...
                projectServers: {
                  ...prev.projectServers,
                  [projectDir]: {
                    ...(prev.projectServers[projectDir] || {}),
                    [name]: config,
                  },
                },
              }
      ...
      if (setMcpServers) {
        setMcpServers((prev) => {
          if (projectDir) {
            ...
            return {
              ...prev,
              projectServers: { ...prev.projectServers, [projectDir]: scoped },
            };
          }
  ...
  const handleReloadServer = useCallback(
    (name: string) => {
      ...
      setServersTools((prev) => ({
        ...prev,
        [name]: { tools: null, loading: true, error: null },
      }));
      void api
          ...
          {isListSidebarOpen && (
            ...
            >
              <div className="flex items-center justify-between p-2 h-10 bg-bg-primary-light border-b border-border-dark-light flex-shrink-0">
                <h3 className="text-sm font-semibold uppercase">{t('settings.tabs.mcpServers')}</h3>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0">
        {isMobile && (
          <div className="flex items-center px-2 py-1.5 border-b border-border-default-dark bg-bg-primary-light flex-shrink-0">
            ...
            <span className="text-sm font-medium uppercase truncate">{selectedServer?.name || t('mcp.selectServer')}</span>
          </div>
        )}
        {selectedServer ? (
          <McpServerToolsPanel
