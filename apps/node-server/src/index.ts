#!/usr/bin/env node
/**
 * AiderDesk Node Server
 *
 * Standalone Node.js server for running AiderDesk without Electron.
 * This provides the HTTP/WebSocket API for interacting with Aider.
 */

import { normalizeBaseDir } from '@aider-desk/common/utils';
import { ProjectData } from '@aider-desk/common/types';
...
import {
  ...
  ModelManager,
  AgentProfileManager,
} from '@aider-desk/core';

const addProjectsFromEnv = async (store: Store, modelManager: ModelManager, agentProfileManager: AgentProfileManager): Promise<void> => {
...
const main = async (): Promise<void> => {
  // Force headless mode for node-server
  if (!process.env.AIDER_DESK_HEADLESS) {
    process.env.AIDER_DESK_HEADLESS = 'true';
  }

  logger.info('------------ Starting AiderDesk Node Server... ------------');
